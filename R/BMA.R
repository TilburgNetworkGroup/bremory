...
bma <- function(smm = NULL,
    ...
    max_width <- max(na.omit(as.vector(smm$intervals$widths)))
...
#'
#' @export
plot.bma <- function(bma, ci = FALSE, xlim, width, height, ncol, nrow, save = TRUE,  widthMax = NULL, output_folder_name = "output_plots"){
    ...
    for(u in 1:length(var_names)){
        for(w in 1:length(weights)){
            ...
            if(u > P){
                ...
                density_intercept <- density(data_loc$effect)
            ...
            #for(u in 1:(dim(draws$posterior_draws[[weights[w]]])[2]-1)){
                ...
                data_median <- apply(df_loc,2,function(x) quantile(x,c(0.5),na.rm=TRUE))
                ...
                data_loc <- data.frame(time = na.omit(bma$elapsedTime), 
                                        lb = df_data_loc[1,],
                                        mode=map_loc, 
                                        ub = df_data_loc[2,],
                                        median = data_median,
